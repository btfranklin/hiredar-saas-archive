...
def sanitize_xml(xml_content: str) -> str:
    ...
    2. Ensures the XML has a root <resume> element
    3. Ensures the XML ends with the closing </resume> tag
    ...
    if not xml_content.strip().startswith("<resume>"):
        ...
        logger.debug("Sanitization: Added missing root <resume> element")
    ...
    if not xml_content.strip().endswith("</resume>"):
        ...
        if "</resume>" not in xml_content:
            ...
            logger.debug("Sanitization: Added missing closing </resume> tag")
